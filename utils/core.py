...
from data import COCODetection, VOCDetection, MOTDetection, preproc
...
def get_dataloader(cfg, dataset, setname='train_sets'):
    ...
    Dataloader_function = {'VOC': VOCDetection, 'COCO': COCODetection, 'MOT': MOTDetection}
    _Dataloader_function = Dataloader_function[dataset]
    if setname == 'train_sets':
        dataset = _Dataloader_function(cfg.COCOroot if dataset == 'COCO' else cfg.VOCroot if dataset == 'VOC' else cfg.MOTroot,
